...
| `src/craft_render.{c,h}` | **X-ray near walls** (ROGUE_FULLFRAME_RENDER): `craft_render_set_xray(x,feet,z,radius)` marks camera-side wall cells within a sphere of the hero; the DDA passes through them (TraceHit.passed_xray) and render_strip veils the pixel, so near walls turn translucent and the hero is never lost behind them. Floor cells (by<feet) and walls beyond the hero stay solid. | polish |
